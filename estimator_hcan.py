...
def main(train=False, eval=False, data_dir=None, model_dir=None, epochs=None, inpfx=''):
    ...
    if train:
        ...
        print("\nTraining...")
        _input_fn = make_input_fn(reviews, labels, shuffle=True, batch_size=BATCH_SIZE)
    ...
    if eval:
        ...
        print("\nEvaluating...")
        _eval_input_fn = make_input_fn(reviews, labels, shuffle=False, batch_size=BATCH_SIZE)
...
# ------------------------------------------------------------------------------
def make_input_fn(reviews, labels, shuffle=None, batch_size=1):
    def input_fn():
        dataset = tf.data.Dataset.from_tensor_slices(({'x': reviews}, labels))
...
#   doc_debatched = doc_input[0]
    ...
    doc_input = features['x'][0]                                  # of dimension (p_max_sentences, p_max_sentence_words)
